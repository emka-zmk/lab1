...
void Keeper::Load() {
    ...
    while (!is.eof()) {
        ...
        if (sBuff == "novelists:") {
            pNewItem = new novelists();
        }
        else if (sBuff == "poets:") {
            pNewItem = new poets();
        }
        else if (sBuff == "fantasists:") {
            pNewItem = new fantasists();
        }
        if (!pNewItem)
            continue;
        pNewItem->Load(is);
...
void Keeper::PushBack(Print_Edition* pObj) {
    if (pObj == nullptr)
...
void Keeper::operator+=(Print_Edition* pObj) {
    PushBack(pObj);
}
Print_Edition& Keeper::operator[](int nIndex) {
...
void Keeper::ReallocBuff(int nNewCopacity) {
    Print_Edition** pNewBuff = new Print_Edition * [nNewCopacity];
